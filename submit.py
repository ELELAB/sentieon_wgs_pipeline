...
#   submit.py - Generic script for Job submission to Computerome2 HPC (original owner is Nanna Barnkob)
...
def get_args(args=None):
    ...
    parser.add_argument("-mem", "--memory", help="Memory in gb. Default is 150.", default=150)
    ...
    parser.add_argument("-T", "--tunnel", dest="tunnel", action="store_true",
                        ...
                        Default name are sentieonstart2.sh and sentieonstop2.sh located in your $HOME")
    ...
    parser.add_argument("-np", "--nproc", default=38, dest="nproc", help="Number of processors to use. Default is 38")
...
def configure_outfiles(workdir, script, script_name, move_outfiles=False):
    if move_outfiles:
        ...
        if len(script.split('\n')) > 1:
            ...
            if version_suffix == "PSP01":
                  (dirname_g, filename_g)=os.path.split(os.path.abspath(script.split(' ')[2]))
                  sampleid_g=filename_g.split(".G.")[0].split("_")[1]
                  outbase = dirname + '/' + filename.split('.PST02.bam')[0] + '.' + version_suffix + '_' + sampleid_g
...
def write_tunnel(homedir):
    line = "#PBS -l prologue={dir}/sentieonstart2.sh\n" \
           "#PBS -l epilogue={dir}/sentieonstop2.sh\n".format(dir=homedir)
    return line
...
def write_qsub(name, script, out_base, nproc=1, memory=20, walltime='1:00:00', workdir=None, python=3, wait_for=None,
    ...
    if script.split('sentieon_')[-1].split('_frozv3')[0] == 'RNAseq': # ADDED LINE, JV
        bash_profile = 'shared_bash_profile_RNAseq'                   # ADDED LINE, JV
    else:                                                             # ADDED LINE, JV
        bash_profile = 'shared_bash_profile'                          # ADDED LINE, JV

    # CHANGE IN LINE SOURCING THE SHARED BASH PROFILE, JV
    qsub_string += \
        '### Job name (comment out the next line to get the name of the script used as the job name)\n' \
        '#PBS -N {name}\n' \
	'### set umask for .err and .out shareable permissions\n' \
	'#PBS -W umask=002\n' \
        ...
        'source /home/projects/HT2_leukngs/apps/github/shared_utils/{bash_profile}\n' \
        ''.format(extra=extra_PBS, name=name, nproc=nproc, out_base=out_base, memory=memory,
                  walltime=walltime, workdir=workdir, bash_profile=bash_profile) # CHANGED LINE, JV
